...
// --- Reviews data ---
// Add/replace items below with your actual quotes.
// Keep < 25 words if you plan to quote verbatim from a source.
const REVIEWS = [
  // Verified rating slide (Justdial)
  {
    text: "Rated 4.3/5 based on customer ratings.",
    name: "Justdial Rating",
    role: "We Care Assurance LLP — Vadodara",
    stars: 5,
    source: { label: "See on Justdial", url: "https://www.justdial.com/Vadodara/We-Care-Assurance-Llp-Old-Padra-Road/0265PX265-X265-230209082348-D7A8_BZDET" }
  },

 {
  text: "Very helpful team. Explained options clearly and handled the claim fast.",
  name: "Amit S.",
  role: "Health Insurance",
  stars: 5,
  source: { label: "View on Google", url: "https://maps.google.com/..." }
}

];

// --- Render into Bootstrap carousel ---
(function renderReviews(){
  ...
  REVIEWS.forEach((r, i) => {
    ...
    const srcHtml = r.source
      ? `<div class="review-source mt-1"><a href="${r.source.url}" target="_blank" rel="noopener">${r.source.label}</a></div>`
      : '';
    ...
    slide.innerHTML = `
      <div class="review-card">
        <div class="review-stars" aria-label="${r.stars || 5} out of 5 stars">${stars}</div>
        <div class="review-text">“${r.text}”</div>
        <div class="review-meta">— ${r.name}${r.role ? ` · ${r.role}` : ''}</div>
        ${srcHtml}
      </div>
    `;
    ...
    ind.type = 'button';
    ind.setAttribute('data-bs-target', '#reviewsCarousel');
    ind.setAttribute('data-bs-slide-to', String(i));
    ind.className = active ? 'active' : '';
